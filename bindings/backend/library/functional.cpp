...
void PyHuskyFunctional::init_py_handlers() {
    ...
    PythonConnector::add_handler("Functional#reduce_by_key", reduce_by_key_sum_handler);
    PythonConnector::add_handler("Functional#reduce_by_key_end", reduce_by_key_sum_end_handler);
...
void PyHuskyFunctional::reduce_by_key_sum_handler(PythonSocket & python_socket, ITCWorker & daemon_socket) {
    // receive name
    std::string name = zmq_recv_string(python_socket.pipe_from_python);
    // create objlist
    auto& reduce_list = ObjListFactory::create_objlist<ReduceObject>(name);
    // create channel
    auto& reduce_ch =
        husky::ChannelFactory::create_push_combined_channel<std::string, husky::SumCombiner<std::string>>(reduce_list, reduce_list, name);
    // receive the num of key-value pairs
    int num = std::stoi(zmq_recv_string(python_socket.pipe_from_python));
    for (int i = 0; i < num; i++) {
        std::string key = zmq_recv_string(python_socket.pipe_from_python);
...
void PyHuskyFunctional::reduce_by_key_sum_end_handler(PythonSocket & python_socket, ITCWorker & daemon_socket) {
    // receive name
    std::string name = zmq_recv_string(python_socket.pipe_from_python);
    // get channel
    auto& reduce_end_ch =  husky::ChannelFactory::get_push_combined_channel<std::string, SumCombiner<std::string>, ReduceObject>(name);
    // get objlist
    auto& reduce_end_list = ObjListFactory::get_objlist<ReduceObject>(name);
    ...
    list_execute(reduce_end_list, [&](ReduceObject & r) {
        ...
        // zmq_send_string(python_socket.pipe_to_python, std::to_string(msgs.size()));
        // for (auto & msg : msgs) {
        zmq_send_string(python_socket.pipe_to_python, msgs);
        //}
    });
